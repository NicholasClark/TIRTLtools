...
## Scenario 1: self-comparison, default settings, backend = "cpu" (numpy, via python)
set.seed(1)
tcr_subset = all_tcrs[sample(nrow(all_tcrs), min(2000, nrow(all_tcrs))), ]
new_res = TCRdist(tcr_subset, tcrdist_cutoff = 90, chunk_size = 400, backend = "cpu")
old_res = TIRTLtools:::TCRdist_old(tcr_subset, tcrdist_cutoff = 90, chunk_size = 400, print_res = FALSE, backend = "cpu")
results["self_cpu_backend"] = .compare_tcrdist_results(old_res, new_res, "self-comparison, backend='cpu'")
...
tcr1 = all_tcrs[idx1, ]
tcr2 = all_tcrs[idx2, ]
new_res = TCRdist(tcr1, tcr2, tcrdist_cutoff = 100, chunk_size = 300, backend = "cpu")
...
new_res = TCRdist(tcr_subset, tcrdist_cutoff = 90, chunk_size = 333, backend = "cpu")
...
new_res = TCRdist(tcr_alpha_only, tcrdist_cutoff = 45, chunk_size = 400, backend = "cpu")
