...
#' @param wellsets a list where each item is a vector of wells to use for the pairing on one plate
...
run_pairing_multiplate = function(
    ...
    min_wells=2,
    well_pos=3,
    wellsets=list(get_well_subset(1:16,1:24)),
    compute=TRUE,
  ...
  mlist_all = lapply(1:length(folder_paths), function(i) {
    ...
    if(verbose) print(msg)
    read_cols = c(targetSequences = "character", aaSeqCDR3 = "character",
                  allVHitsWithScore = "character", allJHitsWithScore = "character",
                  readCount = "numeric", readFraction = "numeric")
    len = length( files[files_bool] )
    if(testing) len = 10
    mlist<-lapply(1:len, function(j) {
      print(j)
  ...
  if(compute==T) {
    ...
    #bigmas_py = r_to_py(bigmas)
    #bigmbs_py = r_to_py(bigmbs)
