...
input_tcrs %>% paged_table()
```

## Run TCRdist

TCRs are processed in chunks (default 1000). For a dataset of this size, computation on a laptop with an Apple Silicon M4 Pro GPU takes about 30 seconds.
...
- `tcr1` - The dataframe with input TCRs, their assigned indices, and any other metadata. The TCR indices, starting from 0, are found in the `tcr_index` column. Note that this dataframe may have fewer rows than the input because some TCRs with non-functional CDR3s or non-standard V-segments may have been removed.
...
edge_df %>% paged_table()
node_df %>% select(tcr_index, everything()) %>% paged_table()

nrow(edge_df) # ~75k edges
```

We detect ~100k relationships between similar TCRs for a TCRdist cutoff of 90.
