# Install python environment

**\[experimental\]** This function install a python environment via the
`basilisk` package that is needed for some functions:
[`TCRdist()`](https://nicholasclark.github.io/TIRTLtools/reference/TCRdist.md),
[`cluster_tcrs()`](https://nicholasclark.github.io/TIRTLtools/reference/cluster_tcrs.md),
and
[`run_pairing()`](https://nicholasclark.github.io/TIRTLtools/reference/run_pairing.md).

It will run automatically when needed, but is available to the user if
needed.

## Usage

``` r
install_python_env(force = FALSE)
```

## Arguments

- force:

  whether to force installation (default if FALSE)

## Value

returns NULL
